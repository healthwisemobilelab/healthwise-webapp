...
type PhlebotomistReportFormProps = {
  initialData: string; // The saved data, as a JSON string
  onSave: (data: ReportData) => void; // Function to call when saving
  onExport: (data: ReportData) => void; // Function to call when exporting
  isSaving: boolean;
};
...
export default function PhlebotomistReportForm({ initialData, onSave, onExport, isSaving }: PhlebotomistReportFormProps) {
  ...
  // When the component loads, try to parse the saved data. If it fails, use a blank form.
  useEffect(() => {
    try {
      const parsedData = JSON.parse(initialData);
      // Ensure the parsed data has the correct structure before setting it
      if (parsedData && typeof parsedData === 'object' && parsedData.specimenTypes) {
        setReportData({ ...blankReport, ...parsedData });
      } else {
        // If the saved data is old (just a string), put it in the notes field
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="border-t pt-6"><h4 className="text-lg font-semibold text-gray-700 mb-2 font-serif">Emergency Contact Details</h4><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><input type="text" name="emergencyContactName" placeholder="Contact Name" value={reportData.emergencyContactName} onChange={handleInputChange} className="w-full p-2 border rounded-md" /><input type="text" name="emergencyContactRelationship" placeholder="Relationship" value={reportData.emergencyContactRelationship} onChange={handleInputChange} className="w-full p-2 border rounded-md" /><input type="tel" name="emergencyContactHomePhone" placeholder="Home Number" value={reportData.emergencyContactHomePhone} onChange={handleInputChange} className="w-full p-2 border rounded-md" /><input type="tel" name="emergencyContactMobile" placeholder="Mobile Number" value={reportData.emergencyContactMobile} onChange={handleInputChange} className="w-full p-2 border rounded-md" /></div></div>
      <div className="border-t pt-6"><label className="block font-semibold text-gray-600 mb-1">Phlebotomist Signature</label><input type="text" name="phlebotomistSignature" placeholder="Type your full name to sign" value={reportData.phlebotomistSignature} onChange={handleInputChange} className="w-full p-2 border rounded-md" /></div>
